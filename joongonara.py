...
def main():
    ...
    start_point = int(file.read().split(":")[1].strip())
    file.close()

    # with open('joongonara.txt', 'w') as outfile:
    crawl_time = time.strftime("%Y/%m/%d %H:%M:%S")
    joongonara.update({"crawl_time": crawl_time})
    for i in range(start_point, articleid_catcher()):
        try:
            ...
            if tree.xpath(('//*[@id="ct"]/p/text()').encode('utf-8')) != [
                u'\uac8c\uc2dc\ubb3c\uc774 \uc874\uc7ac\ud558\uc9c0 \uc54a\uac70\ub098 \uc0ad\uc81c\ub418\uc5c8\uc2b5\ub2c8\ub2e4.']:
                ...
                if tree.xpath('//*[@id="ct"]/div[3]/div[1]/span[1]/text()') != []:
                    ...
                    if ( tree.xpath('//*[@id="ct"]/div[3]/div[1]/span[1]/text()')[0] ).encode('utf-8') == "판매중":
                        ...
                        if tree.xpath('//*[@id="ct"]/p') != None:
                            ...
                            break
            update_last_crawl_articleid = open('joongonara_log.txt', 'w')
            update_last_crawl_articleid.write("articleid : " + i)
            update_last_crawl_articleid.close()
